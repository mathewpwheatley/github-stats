...
function toChartData(
  ...
  statistics.data.forEach((statistic) => {
    Object.entries(statistic.pullRequests.created).forEach(
      ([repository, count]) => {
        addToTotals(totals, 'created', repository, count);
      },
    );

    Object.entries(statistic.pullRequests.merged).forEach(
      ([repository, count]) => {
        addToTotals(totals, 'merged', repository, count);
      },
    );

    Object.entries(statistic.pullRequests.closed).forEach(
      ([repository, count]) => {
        addToTotals(totals, 'closed', repository, count);
      },
    );

    Object.entries(statistic.pullRequests.reviewed).forEach(
      ([repository, count]) => {
        addToTotals(totals, 'reviewed', repository, count);
      },
    );
  });
  ...
  return {
    ...
    datasets: [
      ...
      {
        label: 'Merged',
        data: dataFromTotals(totals, 'merged', labels),
      ...
      },
      {
        ...
        backgroundColor: chartColors.opaque[2],
        borderRadius: 5,
      },
      {
        label: 'Reviewed',
        data: dataFromTotals(totals, 'reviewed', labels),
        backgroundColor: chartColors.opaque[3],
        borderRadius: 5,
      },
    ],
  };
...
export function PullRequestsByRepository(props: {
